<template>
	...
	<!-- Category List -->
		...
		<div class="mt-5 position-relative">
			<div class="panel-head mb-1"><h2 class="title"><small>Category List</small></h2></div>
			<div class="table-responsive">
				<table class="table" ref="table">
					...
					<tr v-for="(category, key) in categories">
						<td>{{ key+1 }}</td>
						<td>{{ category.name }}</td>
						<td><img v-if="categories" :src="base_url+category.image" height="30" :alt="category.name"></td>
						<td>
				          	<div class="btn-group btn-margin justify-content-center w10">
					           	<button @click="erase(category.id)" class="btn btn-trash btn-sml bg-warning delete-alert" title="Delete"><i class="fa fa-trash"></i></button>
...
<script>
	export default{
		...
		mounted(){
			...
			// Get All Category
			this.http('admin/category/all/img', 'category');
		},
		methods:{
			...
			submit:function(event)
			{
				...
				if(this.name != '' && this.file != ''){
					...
					this.loader = true;
					this.http('admin/category/store', 'category', data);
					this.reset();
				}
				else{ warning('Title or image filed is required...') }
			},
			erase:function(id){
				this.http('admin/category/delete/'+id, 'delete');
			},
			close:function(){
			...
			update:function(){
				...
				if(this.e_name != ''){
					...
					this.loader = true;
					this.http('admin/category/update/'+this.id, 'update', data);
			...
			edit:function(index){
				...
				this.e_name 	= this.categories[index].name;
				this.e_img 		= this.categories[index].image;
		...
		watch:{
			kernel:function(){
				for(const key in this.kernel){
					...
					if(key=='delete'){
						this.categories = this.kernel[key];
						success("Your Date Deleted Successfully..");
					}
					else if(key=='update'){
